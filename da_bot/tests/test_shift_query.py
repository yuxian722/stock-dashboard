"""shift_query.py 的離線單元測試(不連網)：mock掉cpis_ee_shift_scraper/
cpis_scraper，驗證即時班別(AD/AN/BD/BN)改機查詢的篩選+組字邏輯，不實際打CPIS。"""
...
class TestLiveGroupShiftChangeoverReply(unittest.TestCase):
    ...
    表單頁面——先試過純urllib模擬POST(cpis_api.py)連續兩輪都回500 Internal
    Server Error，改用無頭瀏覽器實際操作(cpis_ee_shift_scraper.py，跟
    cpis_pm_monitor_scraper.py同款做法)。解析仍用cpis_scraper.
    parse_ee_maintenance_shift_html()(BeautifulSoup解析HTML表格)。
    """

    def setUp(self):
        self._orig_fetch = cpis_ee_shift_scraper.fetch_ee_maintenance_shift_html
        self._orig_parse = cpis_scraper.parse_ee_maintenance_shift_html

    def tearDown(self):
        cpis_ee_shift_scraper.fetch_ee_maintenance_shift_html = self._orig_fetch
    ...
    def test_no_records_for_shift_returns_message(self):
        cpis_ee_shift_scraper.fetch_ee_maintenance_shift_html = lambda *a, **k: "<html>empty</html>"
        cpis_scraper.parse_ee_maintenance_shift_html = lambda html: []
